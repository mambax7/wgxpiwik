...
switch ($op)
{
    ...
    case "defaultblocksettings":
        ...
        unset($result);
        $errors = 0;
        if ($isvisible == 0  || !$blocktitle == '') {
            $sql = "UPDATE " . $xoopsDB->prefix('newblocks') . " SET ('title', 'visible') VALUES ('', 1) WHERE mid = " . $xoopsModule->getVar('mid');
            $result = $xoopsDB->queryF($sql);
            // if (!result == 1) $error++;
            unset($result);
        }
        if (!$bl_module_id == 0) {
            $sql = "UPDATE " . $xoopsDB->prefix('block_module_link') . " SET ('module_id') VALUES (0) WHERE block_id = " . $block_id;
            $result = $xoopsDB->queryF($sql);
            unset($result);
        }
        if ($errors == 0) {
